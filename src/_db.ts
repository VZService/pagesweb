...
export async function ensureSchema(env: Env): Promise<void> {
  ...
  // 以 D1 里的 meta 表为版本权威。KV 只作为快速短路缓存，丢了也不会重跑迁移。
  let applied = "";
  try {
    const row = await env.DB.prepare(`SELECT v FROM meta WHERE k = 'schema_version'`).first<{ v: string }>();
    applied = row?.v ?? "";
  } catch {
    // meta 表刚建出来时理论上不会失败，失败则按未迁移处理
  }
  ...
  try {
    await env.DB.prepare(
      `INSERT INTO meta (k, v, updated_at) VALUES ('schema_version', ?, ?)
